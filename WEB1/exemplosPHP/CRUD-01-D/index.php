...
<?php
    $sql = "";
    if ($tipo == 1){
        $sql = "SELECT * FROM marca WHERE codigo = $procurar ORDER BY codigo";
    }else{    
...
while ($linha = $consulta->fetch(PDO::FETCH_ASSOC)) {
    echo "Código: {$linha['codigo']} - Descrição: {$linha['descricao']}";
    echo " - <a href=javascript:excluirRegistro('acao.php?acao=excluir&id={$linha['codigo']}')>Excluir</a><br>";
}
?>       
</body>
</html>